// ローカルストレージに授業時間の情報を保存
// URLを取得
let url = new URL(window.location.href);
// URLSearchParamsオブジェクトを取得
let params = url.searchParams;
classTimes = params.get('classTimes') 
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    facultyList.addEventListener("click", function(event) {
        ...
        if (clickedFaculty) {
            ...
            window.location.href = `https://ichiken26.github.io/search_class/choicetime/classtable?faculty=${clickedFaculty}`;
        }
    });
});
