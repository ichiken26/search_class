document.addEventListener("DOMContentLoaded", async function() {
    ...
    const selectedClass = urlParams.get("classroom");
    ...
    async function fetchClassData() {
        ...
        for (let i = 1; i < lines.length; i++) { // ヘッダーをスキップ
            ...
            const lastColumn = columns[columns.length - 1]; // 最後のカラムを取得
            const cleanLastColumn = lastColumn.replace(/\r/g, ""); // \r を除去
            ...
            if (columns.length >= 12 && columns[0] === selectedFaculty && columns[1] === selectedBuilding && columns[2] === selectedFloor && columns[3] === selectedClass) {
                classInfo = {
                    periods: columns.slice(6, columns.length - 1).concat(cleanLastColumn) // 最後のカラムを除いた部分に除去済みのカラムを追加
                };
                break;
            }
        }
